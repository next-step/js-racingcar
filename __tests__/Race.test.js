import { Car } from '../src/car'
import { Race } from '../src/race'
import { ERROR_MESSAGE, DEFAULT_MAX_MATCH_LENGTH } from '../src/constants'

describe('Race 1차 요구사항', () => {
  test('경주에 참여한 여러 개의 객체들을 출발선에 놓는다.', () => {
    // Given
    const car1 = new Car('car1')
    const car2 = new Car('car2')
    const car3 = new Car('car3')
    car1.run(4)
    car2.run(4)
    car3.run(4)

    // When
    new Race({
      participants: [car1, car2, car3]
    })

    expect(car1.getPosition()).toBe(0)
    expect(car2.getPosition()).toBe(0)
    expect(car3.getPosition()).toBe(0)
  })

  test('경주에 참여한 객체에 getName 메서드가 포함되어 있지 않은 경우, 해당 메서드가 없다는 에러가 발생한다.', () => {
    // Given
    const inValidObject = {
      run: () => {},
      getPosition: () => {},
      setPosition: () => {}
    }
    const car1 = new Car('car1')

    // When, Then
    expect(() => {
      new Race({
        participants: [inValidObject, car1]
      })
    }).toThrow(new Error(ERROR_MESSAGE.NOT_INCLUDE_METHOD))
  })

  test('경주에 참여한 객체에 getPosition 메서드가 포함되어 있지 않은 경우, 해당 메서드가 없다는 에러가 발생한다.', () => {
    // Given
    const inValidObject = {
      run: () => {},
      getName: () => {},
      setPosition: () => {}
    }
    const car1 = new Car('car1')

    // When, Then
    expect(() => {
      new Race({
        participants: [inValidObject, car1]
      })
    }).toThrow(new Error(ERROR_MESSAGE.NOT_INCLUDE_METHOD))
  })

  test('경주에 참여한 객체에 setPosition 메서드가 포함되어 있지 않은 경우, 해당 메서드가 없다는 에러가 발생한다.', () => {
    // Given
    const inValidObject = {
      run: () => {},
      getName: () => {},
      getPosition: () => {}
    }
    const car1 = new Car('car1')

    // When, Then
    expect(() => {
      new Race({
        participants: [inValidObject, car1]
      })
    }).toThrow(new Error(ERROR_MESSAGE.NOT_INCLUDE_METHOD))
  })

  test('경주에 참여한 객체에 run 메서드가 포함되어 있지 않은 경우, 해당 메서드가 없다는 에러가 발생한다.', () => {
    // Given
    const inValidObject = {
      getName: () => {},
      getPosition: () => {},
      setPosition: () => {}
    }
    const car1 = new Car('car1')

    // When, Then
    expect(() => {
      new Race({
        participants: [inValidObject, car1]
      })
    }).toThrow(new Error(ERROR_MESSAGE.NOT_INCLUDE_METHOD))
  })

  test('경주의 최대 경기 횟수를 정할 수 있다.', () => {
    // Given
    const car1 = new Car('car1')
    const car2 = new Car('car2')
    const car3 = new Car('car3')

    // When
    const race = new Race({
      participants: [car1, car2, car3],
      maxMatchLength: DEFAULT_MAX_MATCH_LENGTH
    })

    // Then
    expect(race.getMaxMatchLength()).toBe(DEFAULT_MAX_MATCH_LENGTH)
  })

  test('경주의 최대 경기 횟수가 숫자가 아닌 경우, 에러를 발생시킨다.', () => {
    // Given
    const car1 = new Car('car1')
    const car2 = new Car('car2')
    const car3 = new Car('car3')

    // When, Then
    expect(() => {
      new Race({
        participants: [car1, car2, car3],
        maxMatchLength: null
      })
    }).toThrow(new Error(ERROR_MESSAGE.NOT_VALID_MATCH_LENGTH))
  })

  test('경주의 최대 경기 횟수가 정의되지 않은 경우, 5회로 지정한다.', () => {
    // Given
    const car1 = new Car('car1')
    const car2 = new Car('car2')
    const car3 = new Car('car3')

    // When
    const race = new Race({
      participants: [car1, car2, car3]
    })

    // Then
    expect(race.getMaxMatchLength()).toBe(DEFAULT_MAX_MATCH_LENGTH)
  })

  test('경주가 시작될 때, 경주에 참여한 객체들을 최대 경기 횟수 동안 앞으로 전진시킨다.', () => {
    // Given
    const car1 = new Car('car1')
    const car2 = new Car('car2')
    const car3 = new Car('car3')
    const race = new Race({
      participants: [car1, car2, car3]
    })

    // When
    race.start()

    // Then
    expect(car1.getPosition()).toBe(5)
    expect(car2.getPosition()).toBe(5)
    expect(car3.getPosition()).toBe(5)
  })

  test('경주에 참여한 객체들의 목록을 출력할 수 있다.', () => {
    // Given, When
    const car1 = new Car('car1')
    const car2 = new Car('car2')
    const car3 = new Car('car3')
    const participants = [car1, car2, car3]

    const race = new Race({
      participants
    })

    // Then
    expect(race.getParticipants()).toBe(participants)
  })

  test('경주에 참여한 객체들의 목록에 직접 접근할 수 없다.', () => {
    // Given
    const car1 = new Car('car1')
    const car2 = new Car('car2')
    const car3 = new Car('car3')
    const race = new Race({
      participants: [car1, car2, car3]
    })

    // When, Then
    expect(() => {
      race.participants
    }).toThrow(new Error(ERROR_MESSAGE.NOT_ACCESS_PARTICIPANTS))
  })

  test('경주에 참여한 객체들의 목록을 직접 변경할 수 없다.', () => {
    // Given
    const car1 = new Car('car1')
    const car2 = new Car('car2')
    const car3 = new Car('car3')
    const race = new Race({
      participants: [car1, car2, car3]
    })

    // When, Then
    expect(() => {
      race.participants = []
    }).toThrow(new Error(ERROR_MESSAGE.NOT_ASSIGN_PARTICIPANTS))
  })

  test('경주가 끝난 뒤, 경주의 우승자를 출력할 수 있다.', () => {
    // Given
    const logSpy = jest.spyOn(console, 'log')
    const car1 = new Car('car1')
    const car2 = new Car('car2')
    const car3 = new Car('car3')
    const race = new Race({
      participants: [car1, car2, car3]
    })

    // When
    race.start()
    race.end()
    console.log(race.getWinners())

    // Then
    const TOTAL_RACE_LENGTH = 15
    expect(logSpy.mock.calls[TOTAL_RACE_LENGTH][0]).toEqual([
      'car1',
      'car2',
      'car3'
    ])
    logSpy.mockRestore()
  })
})
