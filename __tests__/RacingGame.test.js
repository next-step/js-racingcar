...
describe('RacingGame 테스트', () => {
  ...
  describe('레이싱 게임 세팅 테스트', () => {
    test('경주에 참여할 자동차들이 있다.', () => {
      expect(racingGame.carList instanceof CarList).toBe(true);
    });
  });
  ...
  describe('자동차 게임 전진 테스트', () => {
    test('자동차 전진 조건을 만족하면 자동차는 전진한다.', () => {
      ...
      expect(
        racingGame.carList.cars.every((car) => car.distanceDriven > 0)
      ).toBe(true);
    });
  });
  ...
  describe('레이싱 게임 우승 테스트', () => {
    ...
    describe('우승자가 여러명 일 때', () => {
      test('최대 이동거리와 같은 자동차의 수가 2대이면 우승 자동차의 수는 2대다.', () => {
        racingGame.carList.cars[0].moveForward();
        racingGame.carList.cars[1].moveForward();

        expect(
          racingGame.carList.cars.filter(
            (car) => car.distanceDriven === racingGame.carList.maxDistanceDriven
          ).length
        ).toBe(2);
      ...
      test('자동차의 이동거리가 모두 최대 이동거리면 우승자는 모든 자동차다.', () => {
        racingGame.carList.cars.forEach((car) => {
          car.moveForward();
        });

        expect(
          racingGame.carList.cars.every(
            (car) => car.distanceDriven === racingGame.carList.maxDistanceDriven
          )
        ).toBe(true);
        expect(racingGame.winningCars).toEqual(racingGame.carList.cars);
      });
    });
  });
  ...
  describe('경주 라운드 테스트', () => {
    ...
    describe('생성시 maxRounds가 설정 된다면 해당 maxRounds 만큼 진행한다.', () => {
      test.each([6, 8, 10])(
        ...
        async (maxRounds) => {
          racingGame = new RacingGame(carNames, maxRounds);
          const runRoundSpy = jest.spyOn(racingGame, 'runRound');
  ...
  describe('예외처리 테스트', () => {
    describe('생성자 maxRound 테스트', () => {
      test.each(['abc', '1', null, {}])(
        '숫자가 아닐 때',
        (invalidMaxRounds) => {
          expect(() => new RacingGame(carNames, invalidMaxRounds)).toThrow();
        }
      );
    });
  });
