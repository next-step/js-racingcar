    ...
    describe("> 생성할 때", () => {
      ...
      it("라운드 수가 1 미만이면 예외를 발생시켜야 한다", () => {
        expect(() => new Race(cars, 0)).toThrow(
          Race.ERROR_MESSAGES.INVALID_ROUNDS,
        );
      });
    });
  });
  ...
  describe("=== 레이스 기록 테스트 ===", () => {
    describe("> 시작되면 ", () => {
      it("레이스 결과가 라운드 수만큼 기록되어야 한다", () => {
        ...
        expect(raceResult.raceHistory).toHaveLength(race.rounds);
      });

      it("각 라운드마다 자동차의 상태가 기록되어야 한다", () => {
        const raceResult = race.start();
        expect(
          raceResult.raceHistory.every((roundData) =>
            Array.isArray(roundData.cars),
          ),
        ).toBe(true);
      });
    });
