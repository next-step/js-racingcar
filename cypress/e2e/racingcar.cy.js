...
describe('레이싱카 사이트 E2E 테스트', () => {
  ...
  //     cy.startRace(CAR_NAME.DIVERSE_CAR_NAME, MOVE_NUMBER);
  ...
  context('주어진 횟수 동안 n대의 자동차는 전진 또는 멈출 수 있다.', () => {
    ...
    it('전진하는 경우 화살표가 추가 되어야한다.', () => {
      //mocking도 되지 않음 ;;.호출을 읽지 못함. stub이 액션도중 풀려버림.. proxy였다가 일반함수로 ..
      cy.stub(utils, 'getProgressOrNot', () => {
        return true;
      });
      cy.stub(Math, 'random').returns(5);
      cy.get($element.carNameInput).type(CAR_NAME.DIVERSE_CAR_NAME.join(','));
      cy.get($element.nameSubmitButton).click();
      cy.get($element.movesInput).type(MOVE_NUMBER);
      cy.get($element.moveSubmitButton).click();
      // expect(utils.getProgressOrNot).to.be.called;
  ...
  context(
    ...
    () => {
      ...
      it('하나의 자동차가 제출한 이동 횟수에 도달하게 되면 최종우승자를 알려줘야한다.', () => {
        //makeNewRacingMap을 모킹해서 미리 결과를 밀어넣으면 될 것 같은데 stub이 안된다.
        cy.startRace(CAR_NAME.DIVERSE_CAR_NAME, MOVE_NUMBER);
      });
      ...
      it('최종우승자를 알게되면 다시 시작하기 버튼이 생성되어야 한다.', () => {
        cy.startRace(CAR_NAME.DIVERSE_CAR_NAME, MOVE_NUMBER);
        cy.clock();
        cy.tick(WAITING_TIME * MOVE_NUMBER);
        cy.get($element.winnerName).should('exist');
