...
describe('자동차 경주 게임을 실행한다', () => {
  ...
  it('자동차의 이름을 입력하지 않으면 alert 창이 뜬다', () => {
    cy.alertMessage(carNameButtonSelector, ERROR_MESSAGE.LENGTH);
  });
  ...
  it('시도 횟수는 1보다 작은 수를 입력하면 alert 창이 뜬다', () => {
    cy.get(carNameSelector).type('Car1,Car2,Car3,Car4');
    cy.get(carNameButtonSelector).click();
    cy.get(trialNumberSelector).type('0');
    cy.alertMessage(trialNumberButtonSelector, ERROR_MESSAGE.RANGE_NUMBER);
  });
