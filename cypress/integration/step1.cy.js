...
describe('자동차에 이름을 부여할 수 있다.', () => {
 it('자동차 이름을 적을 수 있는 Input이 존재한다.', () => {
  cy.get(carNameInput).should('exist').should('be.visible');
 });
 ...
 it('자동차 이름은 쉼표(,)를 기준으로 구분하며 이름은 5자 이하만 가능하다.', () => {
  cy.get(carNameFieldset).within(() => {
   cy.get(carNameInput).type('kia,메르세데스벤츠,audi,tesla,volvo');
   cy.get('button').click();
   cy.on('window:alert', (str) => {
    expect(str).to.equal(
     ' 유효하지 않은 이름 길이입니다. 자동차의 이름은 1자이상, 5자 이하만 가능합니다.'
 ...
 it('자동차 이름을 타이핑 후 확인 버튼을 누르면, 자동차 이름 input 은 disabled 된다.', () => {
  cy.get(carNameFieldset).within(() => {
   cy.get(carNameInput).type('kia,benz,audi,tesla,volvo');
   cy.get('button').click();
   cy.get(carNameInput).should('be.disabled');
  });
 });
});
