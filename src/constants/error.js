export const ERROR_CODES = {
  ERROR_LONG_CAR_NAME: "ERROR_LONG_CAR_NAME",
  ERROR_EMPTY_CAR_NAME: "ERROR_EMPTY_CAR_NAME",
  ERROR_DUPLICATE_CAR_NAME: "ERROR_DUPLICATE_CAR_NAME",
  ERROR_INVALID_CAR_NAME: "ERROR_INVALID_CAR_NAME",
  ERROR_EMPTY_INPUT_LEN: "ERROR_EMPTY_INPUT_LEN",
  ERROR_INPUT_QUERY_TYPE: "ERROR_INPUT_QUERY_TYPE",
  ERROR_INVALID_RACE_ROUND: "ERROR_INVALID_RACE_ROUND",
  ERROR_EMPTY_RACE_ROUND: "ERROR_EMPTY_RACE_ROUND",
  ERROR_UNKOWN: "ERROR_UNKOWN",
};

export const ERROR_MESSAGES = {
  [ERROR_CODES.ERROR_LONG_CAR_NAME]: "자동차 이름은 5자 이하로 입력해주세요.",
  [ERROR_CODES.ERROR_EMPTY_CAR_NAME]: "자동차 이름을 입력해주세요.",
  [ERROR_CODES.ERROR_DUPLICATE_CAR_NAME]: "중복된 자동차 이름이 있습니다.",
  [ERROR_CODES.ERROR_INVALID_CAR_NAME]: "유효하지 않은 자동차 이름입니다.",
  [ERROR_CODES.ERROR_EMPTY_INPUT_LEN]: "arguments must be 1",
  [ERROR_CODES.ERROR_INPUT_QUERY_TYPE]: "query must be string",
  [ERROR_CODES.ERROR_INVALID_RACE_ROUND]: "양의 정수를 입력해주세요,",
  [ERROR_CODES.ERROR_EMPTY_RACE_ROUND]: "횟수를 입력해주세요.",
  [ERROR_CODES.ERROR_UNKOWN]: "알 수 없는 오류가 발생했습니다.",
};
