...
import { MAX_RACING_ROUNDS } from "../data/constants";
...
export default class RacingController {
  ...
  initCarNamesBeforeStartRacingGame() {
    readlineInterface.question("자동차의 이름을 입력하세요. (각각의 이름은 콤마로 구분합니다.): ", (carNames) => {
  ...
  startRacingGame() {
    ...
    for (let i = 0; i < MAX_RACING_ROUNDS; i++) {
      carList.forEach((_, carIndex) => {
        ...
        if (randomValue >= 4) this.moveCar(carIndex);
      });
