...
describe("Together RacingGame (여럿이서 하는 레이싱 게임)", () => {
  ...
  describe('자동차 게임 시도 횟수를 입력받았을 때', () => {
    it(`자동차 게임 시도 횟수를 입력받아서 자동차 게임을 셋팅할 때, 양의 정수가 아닌 값이 입력됐을 경우 에러를 출력하고 게임을 종료한다.`, () => {
  ...
  describe(`"${DEFAULT_CAR_NAMES_INPUT}"가 레이싱 게임에 등록했을 때`, () => {
  ...
  describe("자동차 경주 진행 중", () => {
    DEFAULT_PLAYERS.forEach((_, playerIdx) => {
      describe(`${START_POSITION}에서 시작해서 ${DEFAULT_GAME_SIZE}회 동안 자동차가 ${CAN_GO_ROUNDS[playerIdx]}번 갈 수 있었을 때`, () => {
