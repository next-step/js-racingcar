...
export class CarFactory {
  ...
  validateCarName(carName) {
    if (isEmptyString(carName)) {
      throw new Error(CAR_FACTORY_ERROR_MESSAGES.CAR_NAME_IS_EMPTY);
    }

    if (carName.length > MAX_CAR_NAME_LENGTH) {
      throw new Error(CAR_FACTORY_ERROR_MESSAGES.CAR_NAME_LOGGER_THAN_MAX_LENGTH);
    }
  }

  validateCarNames(carNames) {
    if (isDuplicated(carNames)) {
      throw new Error(CAR_FACTORY_ERROR_MESSAGES.CAR_NAMES_DUPLICATED);
    }
