...
export function RacingCars() {
  ...
  const cars = [];
  const movableStrategy = () =>
    Math.random() * NUMBERS.RANDOM_BOUND >= BOUNDARY.FORWARD;
  ...
  this.winner = () => {
    ...
    return status
      .filter((stat) => stat[FORWARD_INDEX] === maxForward)
...
export const carForwardTemplate = () => {
  ...
  const $forwardContainer = $element(
    TAG.DIV,
    TAG.CLASS,
    CAR_ATTRIBUTE.CAR_FORWARD
  );
...
export const carDelayTemplate = () => {
  ...
  const $delayContainer = $element(
    TAG.DIV,
    TAG.CLASS,
    CAR_ATTRIBUTE.CAR_DELAY_CONTAINER
  );
