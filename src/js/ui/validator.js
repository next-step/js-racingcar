...
export const validateCarNames = () => {
  ...
  if (
    ...
    throw new ValidationError(ALERT_MESSAGE.INVALID.CAR_NAMES_LENGTH);
  }
  if (isDuplicatedCarNames(carNames)) {
    throw new ValidationError(ALERT_MESSAGE.INVALID.DUPLICATED_CAR_NAMES);
  }
};
...
export const validateAttemptTimes = () => {
  ...
  if (!attemptTimes || attemptTimes < CAR_RACING.ATTEMPT_TIMES.MIN) {
    throw new ValidationError(ALERT_MESSAGE.INVALID.ATTEMPT_TIMES);
  }
};
