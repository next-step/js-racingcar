...
describe("GameModel", () => {
  test("게임의 참가는 자동차만 가능하다.", () => {
    ...
    const setParticipants = cars => () => {
      game.participants = cars;
    };
  ...
  test("중복된 이름으로는 게임에 참여가 불가능하다.", () => {
    const game = new GameModel();
    const carsHasDuplicatedNames = [...DEFAULT_NAMES, ...DEFAULT_NAMES].map(
      v => new CarModel(v),
    );
    ...
    expect(setParticipants(carsHasDuplicatedNames)).toThrow();
  });
});
