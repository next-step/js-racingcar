 ...
 * @param {function} formatter 메시지 요소를 포맷 할 사용자 정의 함수
 * @param {boolean} withLineBreak 메시지 요소 사이에 공백을 추가할지 여부
 */
export const printMessageList = (messageList, formatter = String, withLineBreak = true) => {
  messageList.forEach((item) => {
    if (withLineBreak) {
      printMessage();
    }

    printMessage(formatter(item));
  });
};
